...
const GameSceneInner = ({ mode = 'singleplayer', multiplayerData, onExitToMenu }: GameSceneProps) => {
  ...
  return (
    <div className="relative w-full h-screen" style={{ cursor: 'crosshair' }}>
      ...
      {/* 3D Canvas */}
      <Canvas camera={{ position: [0, 3, -5], fov: 70, near: 0.1, far: 500 }} dpr={1} gl={{ antialias: false, powerPreference: 'high-performance', stencil: false, depth: true }} shadows={false} onCreated={({ gl }) => { gl.setClearColor('#0a1525', 1); }} style={{ background: '#0a1525' }}>

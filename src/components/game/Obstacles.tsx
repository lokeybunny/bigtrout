...
const RockObstacle = ({ obstacle, playerPos, onHit }: ObstacleProps) => {
  ...
  useFrame((_, delta) => {
    if (cooldownRef.current > 0) {
      cooldownRef.current = Math.max(0, cooldownRef.current - Math.min(delta, 0.1));
      return;
    }
  ...
  return (
    <group position={[obstacle.position[0], obstacle.position[1], obstacle.position[2]]}>
  ...
  return (
    <group ref={groupRef} position={[obstacle.position[0], obstacle.position[1], obstacle.position[2]]}>
      <mesh position={[0, 0.3, 0]} rotation={[0, 0, 0.2]}>
        <torusGeometry args={[1.5, 0.4, 4, 6, Math.PI]} />
